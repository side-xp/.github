...
- Group using statements by category in the following order:
    ...
    3. Third-party libraries or plugins
    4. Custom
...
- **Always document** a class in-code with a summary comment (`/// <summary>`), starting with "*Represents...*" if applicable
...
- **Prefer using bit shift notation** for flag fields, starting with `1 << 0` (eg. `1 << 1`, `1 << 2`, ...)
...
- **Prefer using object initializers** instead of setting all `public` fields one by one
